...
- **Endpoint:** Use `https://beta.pokeapi.co/graphql/v1beta`.
- **Queries:** Prefer explicit field selection; avoid over-fetching.
- **Caching:** Use persisted cache to avoid repeated network calls.
...
## GraphQL Execution Architecture

- All GraphQL requests MUST go through a single GraphQL client file inside /core/network.
- UI is strictly forbidden from calling GraphQL directly.
- Every request must go through:
  UI -> Controller (ValueNotifier/ChangeNotifier) -> Repository -> GraphQL Client -> API

## Cache Enforcement

- Always implement cache-first strategy:
  1. Read from cache first
  2. Return cached data immediately if available
  3. Fetch from API in background
  4. Update cache and notify UI
- Cache must persist using local storage (Hive or equivalent)
- Never rely on in-memory-only caching
...
- **Source of truth:** Implement the layout exactly as defined in the public Figma link.
- **Reference:** https://www.figma.com/design/XR13JVgGOwGlf8GKnCt2xQ/Pokedex?node-id=0-1&p=f&t=t3lSMaSzQ6D27mMd-0
- **Assets:** Export icons and images from Figma at 1x/2x/3x as needed.
- **Spacing:** Match paddings, margins, and grid rules from the design.
- **Typography:** Use the same font families, weights, and sizes.
- **Colors:** Use the exact color tokens from the design file.
- **Responsiveness:** Preserve the design intent on small and large screens.

## Figma to Flutter Conversion Rule

1. Always analyze layout BEFORE writing code
2. Split screen into components first
3. Each Figma component MUST become a Flutter widget
4. Reusable components go to /shared/widgets
5. Screen-specific widgets go to feature/presentation/widgets
6. Never generate full screens in a single widget
7. Respect spacing, typography, and hierarchy exactly from Figma
8. Use ThemeData instead of hardcoded styles
9. Ensure responsiveness using LayoutBuilder when needed
...
1. List files that will be created or modified
2. Provide full code per file
3. Group by feature

Example order:

- feature/pokemon/presentation/pages/pokemon_list_page.dart
- feature/pokemon/presentation/widgets/pokemon_card.dart
- feature/pokemon/data/repository/pokemon_repository.dart
- feature/pokemon/data/datasource/pokemon_graphql_datasource.dart

## Rules

- Never mix UI + data layer in same file
- Keep widgets under 20-40 lines when possible
- Always isolate API logic in datasource layer
